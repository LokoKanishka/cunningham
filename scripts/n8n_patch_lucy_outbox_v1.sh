...
docker exec -u node "$CONTAINER" n8n export:workflow --id "$WORKFLOW_ID" --output /tmp/lucy_outbox_src.json >/dev/null
...
docker exec -u node "$CONTAINER" n8n import:workflow --input=/tmp/lucy_outbox_patched.json >/dev/null
